#!/bin/bash
# Qwen3 双模型手动管控脚本 (Orin NX 16GB 专用)
...
# 模型文件
INTENT_MODEL="$MODEL_DIR/qwen3-1.7b.Q4_K_M.gguf"
...
start_chat() {
    if pgrep -f "$LLAMA_BIN.*8080" > /dev/null; then
        echo "[闲聊8B] 已在运行 (PID: $(pgrep -f "$LLAMA_BIN.*8080"))"
        return 0
    fi
    echo "[闲聊8B] 启动中..."
    nohup $LLAMA_BIN \
    ...
    sleep 3
    if pgrep -f "$LLAMA_BIN.*8080" > /dev/null; then
        echo "[闲聊8B] ✅ 启动成功 (Port: 8080)"
    else
        echo "[闲聊8B] ❌ 启动失败，查看 $LOG_DIR/chat.log"
        return 1
    fi
}
...
status() {
    ...
    for port in 8081 8080; do
        name=$([ "$port" = "8081" ] && echo "意图(1.7B)" || echo "闲聊(8B)")
